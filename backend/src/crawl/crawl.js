...
import { chromium } from "playwright"; // thay puppeteer
import fs from "fs";
import path from "path";
import { dataPath } from "../utils/dataConfig.js";
...
class crawler {
  async hcmut_crawler() {
    const browser = await chromium.launch({ headless: true }); // thay puppeteer.launch
    const page = await browser.newPage();

    await page.goto("https://internship.cse.hcmut.edu.vn/", {
      waitUntil: "networkidle", // không cần "networkidle0" vì playwright khác
    });
